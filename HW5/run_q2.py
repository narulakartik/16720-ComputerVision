...
h1=forward(x,params_orig,name='layer1',activation=sigmoid)
...
d=probs-y
delta2 = backwards(d,params_orig,'output', linear_deriv)
...
for k,v in params.items():
    ...
    t=v
    v=t+eps
    h=forward(x, params, 'layer1', activation=sigmoid )
    p=forward(h, params, 'output', softmax)
    loss1, acc1= compute_loss_and_acc(y, p)
    
    
    #v=t-eps
    v=t-eps
    h2=forward(x, params, 'layer1', activation=sigmoid )
    p2=forward(h2, params, 'output', softmax)
    loss2, acc2= compute_loss_and_acc(y, p2)
    v=t
    a=np.ones_like(params['grad_'+k])
    params['grad_'+k]=a*(loss1-loss2)/(2*eps)
    ##########################
    ##### your code here #####
    ##########################
